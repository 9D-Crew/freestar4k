...
if efullscreen:
    ...
    rwin = pg.display.set_mode(
        (rwidth, 480), flags=(borderless * pg.NOFRAME) | pg.RESIZABLE
    )
...
def draw_bg(top_offset=0, bh_offset=0, all_offset=0, special=None, box=True):
    ...
    if not special and box:
        xoff = (screenw - 768) // 2
        pg.draw.rect(
            win, box_c[0], pg.Rect(62 + xoff, 90 - all_offset, 622, 310 - bh_offset)
        )
        pg.draw.rect(
            win, box_c[1], pg.Rect(66 + xoff, 94 - all_offset, 614, 302 - bh_offset)
        )
        pg.draw.rect(
            win, box_c[2], pg.Rect(70 + xoff, 98 - all_offset, 606, 294 - bh_offset)
        )
        pg.draw.rect(
            win, box_c[3], pg.Rect(74 + xoff, 102 - all_offset, 598, 286 - bh_offset)
        )
        pg.draw.rect(
            win, box_c[4], pg.Rect(78 + xoff, 106 - all_offset, 590, 278 - bh_offset)
        )
    if special == "al":
        if "oldal" not in old:
            ...
            pg.draw.rect(
                win, bg_c[2], pg.Rect(0, 91 + 48 + 3 - all_offset, screenw, 48 + 3)
            )
    if special == "df":
        xoff = (screenw - 768) // 2
        adjust = -6
        for i in range(4):
            ...
            pg.draw.rect(
                ...
                box_c[0],
                pg.Rect(
                    62 + xoff, 90 - all_offset + (77 + yoo) * i + 9, 622, 76 + adjust
                ),
            )
            pg.draw.rect(
                win,
                box_c[1],
                pg.Rect(
                    65 + xoff, 93 - all_offset + (77 + yoo) * i + 9, 616, 70 + adjust
                ),
            )
            pg.draw.rect(
                win,
                box_c[2],
                pg.Rect(
                    68 + xoff, 96 - all_offset + (77 + yoo) * i + 9, 610, 64 + adjust
                ),
            )
            pg.draw.rect(
                win,
                box_c[3],
                pg.Rect(
                    71 + xoff, 99 - all_offset + (77 + yoo) * i + 9, 604, 58 + adjust
                ),
            )
            pg.draw.rect(
                win,
                box_c[4],
                pg.Rect(
                    74 + xoff, 102 - all_offset + (77 + yoo) * i + 9, 598, 52 + adjust
                ),
            )
...
def draw_ldl(top_offset=0, bh_offset=0, all_offset=0):
    pg.draw.rect(
        win,
        ldl_c,
        pg.Rect(0, 400 - all_offset - bh_offset, screenw, 80 + all_offset + bh_offset),
    )
    pg.draw.rect(
        win, (33, 26, 20), pg.Rect(0, 400 - all_offset - bh_offset, screenw, 2)
    )
    pg.draw.rect(
        win, (230, 230, 230), pg.Rect(0, 402 - all_offset - bh_offset, screenw, 2)
    )
...
def draw_banner(top_offset=0, bh_offset=0, all_offset=0):
    ...
    pg.draw.polygon(
        ...
        outer_c,
        [
            [screenw - 148 - top_offset, -all_offset],
            [screenw, -all_offset],
            [screenw, 90 - all_offset],
            [screenw - 238 - top_offset, 90 - all_offset],
        ],
    )
...
switches = [
    ...
    False,
    False,
]  # this emulates the 4000's logic switches. why? as baldi once said, historicality
...
def getdata():
    ...
    while True:
        ...
        try:
            wxdata = r.get(
                f"https://wx.lewolfyt.cc/?loc={loc}"
                + ("" if not metric else "&units=m")
                + "&extendeddays=10"
            ).json()
            ...
            if icontable[wxdata["current"]["info"]["iconCode"]] is None:
                ...
                micon = pg.image.load_animation(
                    f"icons_cc/{icontable[wxdata['current']['info']['iconCode']]}.gif"
                )
                nmicon = []
            ...
            if regionalicontable[wxdata["extended"]["daypart"][dn]["iconCode"]] is None:
                ...
                micon = pg.image.load_animation(
                    f"icons_reg/{regionalicontable[wxdata['extended']['daypart'][dn]['iconCode']]}.gif"
                )
                nricon = []
                for fr, ftime in micon:
            ...
            if "df" in flavor:
                for i in range(12):
                    ic = regionalicontable[
                        wxdata["extended"]["daypart"][i + 4]["iconCode"]
                    ]
                    if ic:
                        dficons[i] = [
                            (s.convert_alpha(), ft)
                            for s, ft in pg.image.load_animation(f"icons_reg/{ic}.gif")
                        ]
                    else:
                        dficons[i] = []
                ...
                for alert in aas:
                    az[1].append(
                        (
                            ...
                            alert["rank"],
                            None
                            if alert["alertid"] not in almap
                            else almap[alert["alertid"]],
                        )
                    )
            ...
            for i in range(6):
                ...
                if xficontable[wxdata["extended"]["daypart"][ix]["iconCode"]] is None:
                    ...
                    xficon = pg.image.load_animation(
                        f"icons_xf/{xficontable[wxdata['extended']['daypart'][ix]['iconCode']]}.gif"
                    )
                    ficon = []
        ...
        if "ti" in flavor:
            if not "al" in flavor:
                ...
                if wxdata:
                    lat, long = wxdata["current"]["info"]["geocode"]
                    sr1 = r.get(
                        f"https://api.sunrisesunset.io/json?lat={lat}&lng={long}&time_format=unix"
                    ).json()["results"]
                    sr2 = r.get(
            ...
            t1data = r.get(f"https://wx.lewolfyt.cc/tides?id={tidal[0]}").json()[
                "tides"
            ]
            t1list = {"lows": [], "highs": []}
            for i in range(4):
                fts = dt.datetime.fromtimestamp(t1data[i]["valid"])
                t1list[["lows", "highs"][t1data[i]["type"] == "H"]].append(
                    (
                        splubby_the_return(
                            fts.strftime("%I:%M%p").lower() + " " + fts.strftime("%a")
                        ),
                        t1data[i]["valid"],
                    )
                )
            ...
            t2 = aldata["tidal"][1]
            t2data = r.get(f"https://wx.lewolfyt.cc/tides?id={tidal[1]}").json()[
                "tides"
            ]
            t2list = {"lows": [], "highs": []}
            for i in range(4):
                fts = dt.datetime.fromtimestamp(t2data[i]["valid"])
                t2list[["lows", "highs"][t1data[i]["type"] == "H"]].append(
                    (
                        splubby_the_return(
                            fts.strftime("%I:%M%p").lower() + " " + fts.strftime("%a")
                        ),
        ...
        if "al" in flavor:
            ...
            moons = sorted(
                [
                    ...
                    (
                        "lq",
                        moon_calc.localtime(moon_calc.next_last_quarter_moon(startdt)),
                    ),
                    (
                        "fq",
                        moon_calc.localtime(moon_calc.next_first_quarter_moon(startdt)),
                    ),
        ...
        def get_obsloc(l):
            ...
            try:
                l[2] = r.get(
                    f"https://wx.lewolfyt.cc/?loc={l[0]}&include=current"
                    + ("" if not metric else "&units=m")
                ).json()
                leds[1] = True
            except:
        ...
        def get_regloc(l):
            ...
            try:
                ...
                l3 = pg.image.load_animation(
                    f"icons_reg/{regionalicontable[l[2]['current']['info']['iconCode']]}.gif"
                )
        ...
        def get_tcfloc(l):
            ...
            try:
                l[2] = r.get(
                    f"https://wx.lewolfyt.cc/?loc={l[0]}&include=extended"
                    + ("" if not metric else "&units=m")
                ).json()
                l3 = pg.image.load_animation(
                    f"icons_reg/{regionalicontable[l[2]['extended']['daypart'][1 + (l[2]['extended']['daypart'][0]['dayOrNight'] == 'D')]['iconCode']]}.gif"
                )
                l[3] = [(l[0].convert_alpha(), l[1]) for l in l3]
                # print("got reg icon")
                leds[1] = True
        ...
        try:
            report = r.get(
                f"https://mesonet.agron.iastate.edu/cgi-bin/afos/retrieve.py?&pil={afos_climate}&center=&limit=1&sdate=&edate=&ttaaii=&order=desc"
            ).text
            ...
            clidata = {
                "month_precip": section,
                "temp_outlook": dev1 * sign(ts),
                "precip_outlook": dev2 * sign(rs),
            }
            datagot = True
            leds[1] = True
        except:
        ...
        if "lr" in flavor:
            try:
                if radar_provider == "apollo":
                    radardt = pg.image.load_animation(
                        BytesIO(
                            r.get(
                                f"http://apollo.us.com:8008/radar_composite_animate.gif"
                            ).content
                        )
                    )
                    radardata2 = []
            ...
            try:
                if radar_provider == "apollo":
                    radardt = pg.image.load(
                        BytesIO(
                            r.get(
                                f"http://apollo.us.com:8008/radar_composite.png"
                            ).content
                        )
                    )
                    radardata2 = []
...
def renderoutline(
    font, text, x, y, width, color=(0, 0, 0), surface=win, og=None, ofw=None
):
    if og is None:
...
def time_fmt(time):
    if time >= minutes:
        return (
            str(int(time / minutes))
            + " minutes "
            + str(int((time / seconds) % 60))
            + " seconds"
        )
    else:
...
def loadjrfont(name):
    surfs = (
        pg.image.load(f"jrfonts/fill/{name}.png").convert_alpha(),
        pg.image.load(f"jrfonts/shadow/{name}.png").convert_alpha(),
    )
...
def drawchar(char, cset, x, y, color, half=0):
    ...
    win.blit(
        ...
        (x, y),
        pg.Rect(
            (ix * 32) % cset.get_width(),
            int(ix * 32 // cset.get_width()) * (cset.get_height() // 6),
            32,
            cheight,
        ),
    )
...
def drawshadow(
    ...
    if jr:
        ...
        if leftalign:
            for i, char in enumerate(text):
                ...
                if variable:
                    offsetx -= (
                        (variable[chars.index(char)] + 1)
                        if variable[chars.index(char)]
                        else 15
                    )
                else:
        ...
        if bs:
            if shadow:
                for i, char in enumerate(text):
                    drawchar(
                        char,
                        sheet[1],
                        x + xx + 2 + char_offsets.get(char, 0),
                        y + 2,
                        None,
                    )
                    if variable:
            ...
            for i, char in enumerate(text):
                drawchar(
                    ...
                    x + xx + 2 + char_offsets.get(char, 0),
                    y + 2,
                    color if type(color) != list else color[i],
                )
                if variable:
                    xx += (
                        (variable[chars.index(char)] + 1)
        ...
        if shadow:
            ...
            for i, char in enumerate(text):
                ...
                drawchar(
                    char,
                    sheet[1],
                    x + fxo[0] + xx + char_offsets.get(char, 0),
                    y + fxo[1],
                    None,
                    half=fxa[1],
                )
    ...
    if mono == 0:
        ...
        surface.blit(
            scalec(
                frender(font, text, True, color),
                font,
                text,
                color,
                (fw if ofw is None else ofw, 1),
            ),
            (x, y),
        )
    else:
        ...
        for i, char in enumerate(text):
            ...
            if not bs:
                surface.blit(
                    og, (x + mono * i + offset + coffset, y + offset + coffset2)
                )
            else:
                for j in range(offset):
                    surface.blit(
                        og, (x + mono * i + j + 2 + coffset, y + j + 2 + coffset2)
                    )
                    surface.blit(og, (x + mono * i + j + 2 + coffset, y + j + coffset2))
                    surface.blit(og, (x + mono * i + j + coffset, y + j + 2 + coffset2))
            renderoutline(
                font, char, x + mono * i + coffset, y + coffset2, 1, og=og, ofw=ofw
            )
        ...
        for i, char in enumerate(text):
            ...
            surface.blit(
                scalec(
                    frender(font, char, True, col),
                    font,
                    char,
                    col,
                    (fw if ofw is None else ofw, 1),
                ),
                (x + mono * i + coffset, y + coffset2),
            )
                ...
                while True:
                    ...
                    if data:
                        ...
                        if args[0] == "cue":
                            ...
                            "cuen'tldl",
                            "f",
                        ]:  # press a smaller f to pay respects to the ldl
                            if not ldlon:
                                reps = 1
                                if len(args) > 1:
                                    try:
                            ...
                            "cuen't",
                            "F",
                        ]:  # i'm not renaming it to cancel, and press F to pay respects
                            if not ldlmode:
                                connsendall(conn, f"accepted\n".encode())
                                ldlmode = True
                            else:
...
def drawpage(lines: list, smalltext="", shift=0, vshift=0):
    ...
    for i, line in enumerate(clines):
        ...
        drawshadow(
            ...
            line,
            80 + xoff,
            109 + linespacing * (i - ss) + ldl_y * 1.25 + vshift,
            3,
            mono=gmono,
            char_offsets={},
        )
...
def drawpage2(lines: list, smalltext="", shift=0):
    ...
    for i, line in enumerate(clines):
        ...
        drawshadow(
            ...
            line,
            80 + xoff,
            lerp(endline, startline, i / (len(lines) - 1)),
            3,
            mono=gmono,
            char_offsets={},
        )
...
def parse_ext_action(action):
    ...
    for act in action:
        if act[0] == "set_variable":
            ...
            value = act[2]
            globals()[varname] = (
                value  # set a variable (e.g. ldlmode = True if an extension wants to manually intervene)
            )
            ...
            globals()[destvar] = (
                value  # get a variable from main program and store it in an extension-accessible variable
            )
...
if screenw > 768:
    radarLeft = pg.transform.scale(
        radarHeader.subsurface(pg.Rect(0, 0, 1, radarHeader.get_height())),
        (m.ceil((screenw - 768) / 2), radarHeader.get_height()),
    )
    radarRight = pg.transform.scale(
        radarHeader.subsurface(
            pg.Rect(radarHeader.get_width() - 1, 0, 1, radarHeader.get_height())
        ),
        (m.ceil((screenw - 768) / 2), radarHeader.get_height()),
    )
if screenw > 768:
    radarLeftC = pg.transform.scale(
        radarHeaderC.subsurface(pg.Rect(0, 0, 1, radarHeaderC.get_height())),
        (m.ceil((screenw - 768) / 2), radarHeaderC.get_height()),
    )
    radarRightC = pg.transform.scale(
        radarHeaderC.subsurface(
            pg.Rect(radarHeaderC.get_width() - 1, 0, 1, radarHeaderC.get_height())
        ),
        (m.ceil((screenw - 768) / 2), radarHeaderC.get_height()),
    )
...
def docapture():
    ...
    while True:
        ...
        if smode == 1:
            scaled = cv2.resize(
                frame2,
                (int(480 * fr_size[1] / fr_size[0] * (4 / 3 if sm2 else 1)), screenw),
            )
        if smode == 2:
            scaled = cv2.resize(
                frame2, (480, int(fr_size[0] / fr_size[1] * (1.2 if sm2 else 1) * 480))
            )
...
def drawreg(surf, pos, ix=0):
    if surf:
        ...
        win.blit(
            pg.transform.smoothscale_by(surf[ix][0], (1.2, 1)),
            (pos[0] - 32, pos[1] - 19),
        )
...
while working:
    for event in pg.event.get():
        ...
        if event.type == pg.KEYDOWN:
            if event.key == pg.K_s:
                ...
                pg.image.save(
                    pg.transform.smoothscale_by(win, (1 / 1.2, 1)),
    ...
    try:
        sanitize = lambda tx: tx.replace("in the Vicinity", "Near").replace(
            "Thunderstorm", "T'Storm"
        )
        ccphrase = sanitize(wxdata["current"]["info"]["phraseLong"])
        if len(ccphrase) > len(
            " Partly Cloudy "
        ):  # sorry i had no better text reference
    ...
    if ldlmode:
        ...
        if ldlfeedactive and latestframe is not None:
            ...
            win.blit(
                latestframe,
                (screenw // 2 - latestframe.width / 2, 240 - latestframe.height / 2),
            )
    else:
        if slide == "oldcc":
            # win.blit(ws1b, (0, 0))
            draw_bg(
                top_offset=(192 * ("fullOldCC" not in old)),
                all_offset=ao,
                bh_offset=ao // 2,
            )
        else:
            ...
            draw_bg(
                all_offset=ao,
                bh_offset=ao // 2,
                special=(slide if slide in spec else None),
                box=(slide != "xf"),
            )
            if slide == "xf":
                for i in range(3 + widescreen):
                    win.blit(
                        xfbg, (46 + 230 * i + 13 * widescreen, 101 - round(ao * 1.5))
                    )
    profiling_end("ops")
        ...
        if slide in ["cc", "oldcc"]:
            if slide == "oldcc":
                ...
                if True:
                    ...
                    if veryuppercase:
                        ln = ln.upper()
                    drawshadow(
                        startitlefont, ln, 194 + txoff // 3, 30, 3, mono=18, ofw=1.07
                    )
                # else:
                ...
                if wxdata is None:
                    ...
                    page = [ccphrase]
                    if (
                        wxdata["current"]["conditions"]["feelsLike"]
                        == wxdata["current"]["conditions"]["temperature"]
                    ):
                        additional = ""
                    elif (
                        wxdata["current"]["conditions"]["feelsLike"]
                        ...
                        additional += (
                            padtext(wxdata["current"]["conditions"]["feelsLike"], 3)
                            + "°"
                            + temp_symbol
                        )
                    else:
                    ...
                    page.append(
                        textmerge(
                            f"Temp:{padtext(wxdata['current']['conditions']['temperature'], 3)}°"
                            + temp_symbol,
                            additional,
                        )
                    )
                    ...
                    wndtext = textmerge(
                        f"Wind: {padtext(wxdata['current']['conditions']['windCardinal'], 3)}",
                    ...
                    if wxdata["current"]["conditions"]["windGusts"] is not None:
                        wndtext = textmerge(
                            wndtext,
                            f"                  Gusts to  {wxdata['current']['conditions']['windGusts']}",
                        )
                    page.append(wndtext)
                    ...
                    if wxdata["current"]["conditions"]["cloudCeiling"] is None:
                        ...
                        ceil = padtext(
                            f"{wxdata['current']['conditions']['cloudCeiling']} {short_dist}",
                            10,
                        )
                    cltext = (
                        f"Visib:  {padtext(int(wxdata['current']['conditions']['visibility']), 2)} {long_dist} Ceiling"
                        + ceil
                    )
                    page.append(cltext)
                ...
                if not wxdata:
                    ...
                    drawshadow(
                        starfont32,
                        f"{padtext(wxdata['current']['conditions']['humidity'], 3)}%",
                        576 + txoff,
                        133 + ldl_y,
                        3,
                        mono=19,
                    ...
                    drawshadow(
                        starfont32,
                        f"{padtext(wxdata['current']['conditions']['dewPoint'], 3)}°",
                        576 + txoff,
                        176 + ldl_y,
                        3,
                        mono=19,
                    ...
                    drawshadow(
                        starfont32,
                        f"  {padtext(round(wxdata['current']['conditions']['visibility']), 2)}{long_dist}",
                        540 + txoff,
                        261 + ldl_y,
                        3,
                        mono=17.5,
                    ...
                    drawshadow(
                        starfont32,
                        "Pressure :"
                        if ("ccspace" in old and not metric)
                        else "Pressure:",
                        394 + txoff,
                        304 + ldl_y,
                        3,
                    ...
                    if additional:
                        ...
                        drawshadow(
                            starfont32,
                            f"{padtext(wxdata['current']['conditions']['feelsLike'], 3)}°",
                            576 + txoff,
                            347 + ldl_y,
                            3,
                            mono=19,
                    ...
                    drawshadow(
                        largefont32,
                        f"{padtext(wxdata['current']['conditions']['temperature'], 3)}°",
                        170 + txoff,
                        99 + ldl_y,
                        3,
                        ofw=1.125,
                    ...
                    mm = pg.transform.smoothscale_by(
                        mainicon[m.floor(iconidx) % len(mainicon)][0], (1.2, 1)
                    )
                    ioff = (0, 0)
                    if icontable[wxdata["current"]["info"]["iconCode"]] in icon_offset:
                        ioff = icon_offset[
                            icontable[wxdata["current"]["info"]["iconCode"]]
                        ]
                    win.blit(
                        mm,
                        (
                            220 - mm.width // 2 + ioff[0] + txoff,
                            215 - mm.height // 2 + ioff[1] + ldl_y,
                        ),
                    )
                    del mm
                    ...
                    if wxdata["current"]["conditions"]["windSpeed"] == 0:
                        ...
                        drawshadow(
                            extendedfont,
                            f"Wind: {padtext(wxdata['current']['conditions']['windCardinal'], 3)}  {padtext(wxdata['current']['conditions']['windSpeed'], 2)}",
                            95 + txoff,
                            303 + ldl_y,
                            3,
                            ofw=1.1,
                    ...
                    if wxdata["current"]["conditions"]["windGusts"] is not None:
                        drawshadow(
                            ...
                            ofw=1,
                            mono=18.9,
                            char_offsets={"u": 3, "s": 2},
                            upper=veryuppercase,
                        )
                        drawshadow(
                            extendedfont,
                            padtext(wxdata["current"]["conditions"]["windGusts"], 2),
                            272 + txoff,
                            345 + ldl_y,
                            3,
                            ofw=1.1,
            ...
            for l in obsloc:
                if l[2] == None:
                    ...
                    ol = textmerge(
                        l[1],
                        f"              {padtext(l[2]['current']['conditions']['temperature'], 3)} {shorten_phrase(l[2]['current']['info']['phraseShort'])}",
                    )
                    ws = l[2]["current"]["conditions"]["windSpeed"]
                    if ws > 9:
                        ol = textmerge(
                            ol,
                            f"                            {windreduce(l[2]['current']['conditions']['windCardinal'])}",
                        )
                        page.append(
                            textmerge(ol, f"                              {ws}")
                        )
                        ...
                        ol = textmerge(
                            ol,
                            f"                            {l[2]['current']['conditions']['windCardinal']}",
                        )
                        page.append(
                            textmerge(ol, f"                               {ws}")
                        )
            ...
            for alert in alerts:
                allines = [
                    lin.strip().rstrip() for lin in wraptext(alert[0].upper(), 30)
                ]
                for line in allines:
                    finallines.append(
                        textmerge(
                            "*" + 30 * " " + "*",
                            m.ceil(16 - len(line) / 2) * " " + line,
                        )
                    )
                finallines.append("\n")
            ...
            if wxdata:
                if not alert36:
                    ...
                    text = (
                        (
                            fcsts[subpage]["name"][0].upper()
                            + fcsts[subpage]["name"][1:].lower()
                        )
                        + "..."
                        + fcsts[subpage]["narration"]
                    )
                    if veryuppercase:
                        text = text.upper()
                    ...
                    for textixx in range(4):
                        texts.append(
                            wraptext(
                                (
                                    fcsts[textixx]["name"][0].upper()
                                    + fcsts[textixx]["name"][1:].lower()
                                )
                                + "..."
                                + fcsts[textixx]["narration"]
                            )
                        )
                        if veryuppercase:
                    ...
                    def build36(squ):
                        ...
                        while True:
                            ...
                            if len(all_lines) % 7 == 0:
                                ...
                                all_lines.append(
                                    (
                                        fcsts[textix]["name"][0].upper()
                                        + fcsts[textix]["name"][1:].lower()
                                    )
                                    + "..."
                                )
            ...
            if aldata["sun"]:
                ...
                drawshadow(
                    starfont32,
                    d2,
                    213 + 286 + 18 * 4.5 - len(d2) * 18 / 2 + txoff,
                    85 + ldl_y,
                    3,
                    mono=gmono,
                    char_offsets={},
                ...
                drawshadow(
                    starfont32,
                    supper(splubby_the_return(sunrise1.strftime("%I:%M %p"))),
                    305 + txoff,
                    114 + ldl_y,
                    3,
                    mono=gmono,
                ...
                drawshadow(
                    starfont32,
                    supper(splubby_the_return(sunset1.strftime("%I:%M %p"))),
                    305 + txoff,
                    144 + ldl_y,
                    3,
                    mono=gmono,
                ...
                drawshadow(
                    starfont32,
                    supper(splubby_the_return(sunrise2.strftime("%I:%M %p"))),
                    518 + txoff,
                    114 + ldl_y,
                    3,
                    mono=gmono,
                ...
                drawshadow(
                    starfont32,
                    supper(splubby_the_return(sunset2.strftime("%I:%M %p"))),
                    518 + txoff,
                    144 + ldl_y,
                    3,
                    mono=gmono,
            ...
            if aldata["moon"]:
                ...
                for i in range(4):
                    ...
                    mn = pg.transform.smoothscale_by(
                        {
                            "New": moon_new,
                            "First": moon_fq,
                            "Full": moon_full,
            ...
            if wxdata:
                for i in range(3 + widescreen):
                    ...
                    ix = (
                        ...
                        + 4
                        + subpage * 6
                        - (wxdata["extended"]["daypart"][0]["dayOrNight"] == "N")
                    )
                    ...
                    drawshadow(
                        largefont32,
                        lo,
                        114 + i * 230 - 18 * 2 + 24 - len(lo) * 12 + to,
                        344 + yo,
                        3,
                        mono=25,
                    )
                    ...
                    drawshadow(
                        largefont32,
                        hi,
                        114 + i * 230 + 18 * 3 + 24 - len(hi) * 12 + to,
                        344 + yo,
                        3,
                        mono=25,
                    )
                    if xficons[i + subpage * 3]:
                        xi = xficons[i + subpage * 3][
                            int(iconidx3 % len(xficons[i + subpage * 3]))
                        ][0]
                        xi = pg.transform.smoothscale_by(xi, (1.2, 1))
                        win.blit(
                            xi,
                            (
                                120 + i * 230 + 27 - xi.get_width() / 2 + to,
                                200 - xi.get_height() / 2 + yo,
                            ),
                        )
                        del xi
            else:
                drawshadow(
                    starfont32, "Temporarily Unavailable", 177, 218, 3, mono=gmono
                )
        elif slide == "ol":
            drawshadow(
                startitlefont,
            ...
            if wxdata is None:
                ...
                for time in times:
                    line = time[1]
                    line = textmerge(
                        line, f"        {shorten_phrase(time[0]['phraseShort'])}"
                    )
                    line = textmerge(
                        line, f"                  {round(time[0]['temperature'])}"
                    )
                    line = textmerge(
                        line,
                        f"                     {min(round(time[0]['rainChance'] + time[0]['snowChance'] + time[0]['sleetChance'] + time[0]['freezingRainChance']), 100)}%",
                    )
                    precip_type = None
                    ch = [
                        (time[0]["snowChance"], "Snow"),
                        (time[0]["sleetChance"], "Sleet"),
                        (time[0]["freezingRainChance"], "FrzRn"),
                        (time[0]["rainChance"], "Rain"),
                    ]
            ...
            if not wxdata:
                ...
                for i in range(4):
                    ...
                    drawshadow(
                        starfont32,
                        ps if len(pl) > 14 else pl,
                        62 + 14 + txoff,
                        96 + 14 + ldl_y + (yoo + 77) * i + 9,
                        3,
                        mono=gmono,
                        upper=veryuppercase,
                    )
                    drawshadow(
                        starfont32,
                        padtext(wxdata["extended"]["daypart"][j]["temperature"], 3),
                        62 + 14 + txoff + 18 * 15,
                        96 + 14 + ldl_y + (yoo + 77) * i + 9,
                        3,
                        mono=gmono,
                    ...
                    drawreg(
                        dficons[j - 4],
                        (640 + txoff - 15, 96 + 14 + ldl_y + (yoo + 77) * i + 20),
                        ix=(m.floor(iconidx3) % len(dficons[j - 4])),
                    )
        elif slide == "intro":
            drawshadow(
                startitlefont,
            ...
            drawshadow(
                largefont32, tx, 98 + txoff, 109 + linespacing / 2 + ldl_y, 3, mono=20
            )
            ...
            for i, line in enumerate(wraptext(introtx, 30)):
                ...
                drawshadow(
                    ...
                    tx,
                    98 + txoff,
                    109 + linespacing * 1.75 + 32 * i + ldl_y,
                    3,
                    mono=gmono,
                    char_offsets={},
                )
            ...
            def drawregloc(name, temp, idx, pos):
                ...
                if reglocs[idx][3]:
                    drawreg(
                        reglocs[idx][3],
                        (xx + 60, yy + 18),
                        (m.floor(iconidx3) % len(reglocs[idx][3])),
                    )
                drawshadow(
                    starfont32,
                    name,
                    xx - round(len(name) * 15 / 2) + 34,
                    yy - 25,
                    3,
                    mono=15,
                )
                drawshadow(
                    ...
                    mono=15,
                    color=yeller,
                    char_offsets=jroffsetstall,
                    jr_override=jrfonttall,
                    variable=jrwidthstall,
                    leftalign=True,
                )
            ...
            for i, lc in enumerate(reglocs):
                drawregloc(
                    lc[1],
                    "" if not lc[2] else lc[2]["current"]["conditions"]["temperature"],
                    i,
                    (150, 150),
                )
        elif slide == "tcf":
                ...
                for i in range(4):
                    win.blit(
                        ...
                        (
                            0,
                            91
                            + (i - 2) * tcf_bg.get_height()
                            + 18
                            + yy % tcf_bg.get_height(),
                        ),
                    )
            else:
                win.fill((0, 0, 64))
            for i, name in enumerate(tcflocs):
                drawshadow(
                    ...
                    color=yeller,
                    mono=19,
                    variable=jrwidthstravel,
                    char_offsets=jroffsetstravel,
                )
                if tcflocs[i][2]:
                    drawshadow(
                        largefont32,
                        padtext(
                            str(tcflocs[i][2]["extended"]["daily"][1]["tempMin"]), 3
                        ),
                        540 - 21,
                        72 * i + 120 + yy,
                        3,
                    ...
                    drawshadow(
                        largefont32,
                        padtext(
                            str(tcflocs[i][2]["extended"]["daily"][1]["tempMax"]), 3
                        ),
                        613 - 21,
                        72 * i + 120 + yy,
                        3,
                ...
                if tcflocs[i][3]:
                    drawreg(
                        tcflocs[i][3],
                        (430, 72 * i + 120 + yy + 20),
                        (m.floor(iconidx3) % len(tcflocs[i][3])),
                    )
            ...
            drawshadow(
                smallfont,
                "HIGH",
                479 + round((screenw - 768) * 2 / 3) + 54 + 66,
                75 + ldl_y,
                3,
                color=yeller,
                mono=gmono,
            ...
            if aldata["tidal"][0]:
                lines[1] = textmerge(
                    textmerge(
                        "Lows:", " " * 7 + padtext(aldata["tidal"][0]["lows"][0][0], 11)
                    ),
            ...
            if aldata["tidal"][1]:
                lines[4] = textmerge(
                    textmerge(
                        "Lows:", " " * 7 + padtext(aldata["tidal"][1]["lows"][0][0], 11)
                    ),
            ...
            if aldata["sun"]:
                ...
                lines[6] = textmerge(
                    textmerge(
                        "   Sunrise         Set",
                        " " * 11 + splubby_the_return(s1.strftime("%I:%M%p").lower()),
                    ),
                    " " * 23 + splubby_the_return(s2.strftime("%I:%M%p").lower()),
                )
    ...
    if (ldlon and not serial) or not ldlmode or alerting:
        if serial:
            drawshadow(
                smallfont,
                "SN: 000000v1.0 SW:00000000 DQ:100",
                78,
                402.5 - 8,
                3,
                mono=gmono,
            ...
            drawshadow(
                smallfont,
                "RLYS:0110 BAUD:9600 SENSORS:N/A",
                78,
                402.5 + 24,
                3,
                mono=gmono,
            ...
            if ldlidx == 0:
                ...
                if wxdata is not None:
                    ...
                    ldltext = textmerge(
                        f"Temp:{padtext(wxdata['current']['conditions']['temperature'], 3)}°{temp_symbol}",
                        additional,
                    )
            elif ldlidx == 3:
                if wxdata is not None:
                ...
                if wxdata is not None:
                    ...
                    if wxdata["current"]["conditions"]["windGusts"] is not None:
                        ldltext = textmerge(
                            ldltext,
                            f"{ldlspace}                  Gusts to  {wxdata['current']['conditions']['windGusts']}",
                        )
            elif ldlidx == 6:
                if wxdata is not None:
                    ceil = f":{padtext(wxdata['current']['conditions']['cloudCeiling'], 5)}{short_dist}"
                    if wxdata["current"]["conditions"]["cloudCeiling"] is None:
                        if "ceiling_colon" in old:
                            ...
                            ceil = " Unlimited"
                    ldltext = (
                        f"Visib:  {padtext(int(wxdata['current']['conditions']['visibility']), 2)} {long_dist} {ldlspace}Ceiling"
                        + ceil
                    )
            elif ldlidx == 7:
                ...
                mm = pg.transform.smoothscale_by(
                    ldllficon[m.floor(iconidx3) % len(ldllficon)][0], (1.2, 1)
                )
                if fc == 0:
                    ...
                    drawshadow(
                        starfont32, "Tomorrow", xx, 409, 3, mono=15, color=yeller
                    )
                    drawshadow(
                        starfont32, "'", xx + 72 + 50, 409, 3, mono=15, color=yeller
                    )
                    drawshadow(
                        starfont32,
                        "s Forecast:",
            ...
            if alerting:
                pg.draw.rect(
                    ...
                    (
                        (187, 17, 0)
                        if (slide not in ["lr", "cr"] or ("warnpalbug" not in old))
                        else (128, 16, 0)
                    )
                    if True or "ADVISORY" not in crawl
                    else (126, 31, 0),
            ...
            if (
                (
                    (slide in ["lr", "cr"])
                    or (colorbug_started and colorbug_nat and ldlmode)
                )
            ...
            if crawlscroll >= (
                screenw + (len(crawl) + 4) * (gmono if not jr else m.floor(gmono))
            ):
                if alerting:
                    crawlscroll = 0
                    alertactive += 1
    ...
    rwin.blit(
        tr,
        (
            rwin.get_width() / 2 - tr.get_width() / 2,
            rwin.get_height() / 2 - tr.get_height() / 2,
        ),
    )
    del tr
    pg.display.flip()
